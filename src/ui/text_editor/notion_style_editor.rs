    ...
    DataState<T, P>: DataHandler,
{
    ...
    // Clone for different usages
    let data_state_for_save = data_state.clone();
    let field_name_for_save = field_name.clone();
    let data_state_for_edit = data_state.clone();
    let field_name_for_edit = field_name.clone();
    let data_state_for_split = data_state.clone();
    let field_name_for_split = field_name.clone();
                    ...
                    <div class="w-full h-screen flex flex-col bg-white">
                        ...
                        <div class="sticky top-0 z-10 bg-white border-b shadow-sm">
                            <div class="flex justify-between items-center p-3">
                                ...
                                // Save indicator
                                <SaveIndicator data_state=data_state_for_save field_name=field_name_for_save />
                            </div>
                        </div>
                        
                        // Content area - clone values for each view
                        <div class="flex-1 overflow-hidden">
                            <Show when=move || view_mode.get() == ViewMode::Edit>
                                <EditView 
                                    value=value.clone() 
                                    data_state=data_state_for_edit.clone() 
                                    field_name=field_name_for_edit.clone() 
                                />
                            </Show>
                            ...
                            <Show when=move || view_mode.get() == ViewMode::Split>
                                <SplitView 
                                    value=value.clone() 
                                    data_state=data_state_for_split.clone() 
                                    field_name=field_name_for_split.clone() 
                                />
                            </Show>
                        </div>
                    </div>
